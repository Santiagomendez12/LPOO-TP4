...
void guardarDatos(Persona persona) {
    ...
    if (duplicado) {
        ...
        archivo = fopen("datos.txt", "a"); // Abre el archivo en modo append (agregar al final)
        if (archivo == NULL) {
...
void buscarPorNombreApellido(char nombreBuscar[], char apellidoBuscar[]) {
    FILE *archivo;
    char nombre[MAX_NOMBRE];
    char apellido[MAX_APELLIDO];
    char dni[MAX_DNI];
    ...
    do {
        ...
        switch (opcion) {
            case 1:
                ...
                guardarDatos(personas[numPersonas]);
                numPersonas++;
                break;
            case 2:
                ...
                fgets(personas[numPersonas].apellido, MAX_APELLIDO, stdin);
                personas[numPersonas].apellido[strcspn(personas[numPersonas].apellido, "\n")] = 0; // Eliminar el salto de línea
                buscarPorNombreApellido(personas[numPersonas].nombre, personas[numPersonas].apellido);
                break;
            case 3:
